...
Matrix createLocalK(int element,mesh &m){
    float D,Ae,E = m.getParameter(THERMAL_CONDUCTIVITY), Av = m.getParameter(Avar);
    Matrix K,A,B,Bt,At;
    ...
    productRealMatrix(-E*Av*Ae/(D*D),productMatrixMatrix(Bt,productMatrixMatrix(At,productMatrixMatrix(A,B,2,2,3),2,2,3),3,2,3),K);
